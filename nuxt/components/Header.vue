...
<template>
  <div>
    <div class="container__header justify-space-between">
      <nuxt-link class="container__nuxt-link" to="/">
        <logo-video-service />
      </nuxt-link>
      ...
      <div v-if="this.$store.state.user" class="d-flex">
        ...
        <v-btn class="container__button" @click="LogoutUser">
          Выйти
        </v-btn>
      </div>

      <div v-else>
        <v-btn class="container__button ma-2" to="/userLogin">
          Войти
        </v-btn>
        <v-btn class="container__button" to="/userRegistration">
          Зарегистрироваться
        </v-btn>
      </div>
    </div>
    ...
    >
      <v-card width="304" class="mx-auto mt-5">
        <v-card-title>
          <h1 class="display-1 font-weight-bold mx-auto">
            Вход
          </h1>
        </v-card-title>
